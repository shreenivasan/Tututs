...
function isLoggedIn() {
    ...
    if (isset($_COOKIE['auth_php_redis'])) {
        $r = redisLink();
        $authcookie = $_COOKIE['auth_php_redis'];    
        $r->hget("auths_php_redis",$authcookie);
        if ($userid = $r->hget("auths_php_redis",$authcookie)) {                
            if ($r->hget("user_php_redis:$userid","auth") != $authcookie) return false;
            loadUserInfo($userid);
            return true;
        }
    }
...
function loadUserInfo($userid) {
    ...
    $r = redisLink();
    $User['id'] = $userid;
    $User['username'] = $r->hget("user_php_redis:$userid","username");
    return true;
}
...
function showPost($id) {
    $r = redisLink();
    $post = $r->hgetall("post_php_redis:$id");
    if (empty($post)) return false;

    $userid = $post['user_id'];
    $username = $r->hget("user_php_redis:$userid","username");
...
function showUserPosts($userid,$start,$count) {
    $r = redisLink();
    $key = ($userid == -1) ? "timeline_php_redis" : "posts_php_redis:$userid";
...
function showLastUsers() {
    $r = redisLink();
    $users = $r->zrevrange("users_by_time_php_redis",0,9);
    echo("<div>");
    foreach($users as $u) {
